...
\begin{enumerate}[label=(\alph*)]
    ...
    \item If $x \in X$, $x' \in X$ and if $\{x_n\}$ converges to $x$ and to $x'$, then $x = x'$.
    
    \item If $\{x_n\}$ converges, then $\{x_n\}$ is bounded.
    
    \item If $E \subset X$ and $x$ is a limit point of $E$, then there is a sequence $\{x_n\}$ in $E$ such that $\lim_{n\to\infty} x_n = x$.
\end{enumerate}
\end{theorem}
\begin{proof}
~\begin{enumerate}[label=(\alph*)]
...
\begin{remark}
Being complete means every Cauchy sequence in $X$ also converges in $X$. Hence, Theorem \ref{th_214} implies that all compact metric and all Euclidean spaces are complete. 
\end{remark}
